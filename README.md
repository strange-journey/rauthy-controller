...
With the controller running, apply an `OIDCClient` resource:

```sh
kubectl apply -f examples/oidcclient.yaml
```
...
kubectl get secret example-webapp-oidc-secret-custom-name -o yaml
kubectl delete oidcclient example-webapp   # triggers cleanup in Rauthy via finalizer
```

See [examples/oidcclient.yaml](examples/oidcclient.yaml) for an annotated example.
...
Portions of the controller boilerplate are derived from the
[controller-rs](https://github.com/kube-rs/controller-rs) reference project.

<details>

<summary>Copyright 2021 The Kube-rs Authors. Apache License 2.0.</summary>
...
```
   ...
   Copyright 2021 The Kube-rs Authors.
